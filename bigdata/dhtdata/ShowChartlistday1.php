...
<?php
	...
	$qry ="select MAC, left(systime,8) as systime, avg(temperature) as temp, avg(temperature) as humid from big.dhtdata where MAC = '%s' and systime >= '%s' and systime <= '%s' group by left(systime,8) order by systime desc " ;		//將dhtdata的資料找出來
